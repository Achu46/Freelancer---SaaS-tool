...
export default function Auth({ mode = 'login' }) {
  ...
  const { login, signup, loginWithGoogle, setUserProfile } = useAuth();
  ...
  async function ensureAdminAccount(uid, email) {
    try {
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      if (user.email === ADMIN_EMAIL) {
        role = await ensureAdminAccount(user.uid, user.email);
      } else {
      ...
      if (err.code === 'auth/unauthorized-domain') {
        toast.error(
          `This domain ("${window.location.hostname}") is not authorized for Google Sign-In. Please add it to the Authorized Domains in your Firebase Authentication Console settings.`,
          { duration: 8000 }
        );
    ...
    try {
      if (tab === 'login') {
        ...
        if (form.email === ADMIN_EMAIL) {
          role = await ensureAdminAccount(result.user.uid, form.email);
        } else {
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-white dark:bg-slate-950">
        <div className="w-full max-w-sm">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-4 py-2.5 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-200 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-all font-semibold text-sm shadow-sm disabled:opacity-60 disabled:cursor-not-allowed hover:-translate-y-0.5 cursor-pointer active:scale-98"
          >
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-white dark:bg-slate-950 px-3 text-slate-500 dark:text-slate-400 font-semibold tracking-wider">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {tab === 'signup' && (
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition text-sm"
                />
              </div>
            )}
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                  minLength={6}
                  className="w-full px-4 py-2.5 pr-10 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition text-sm"
                />
                <button
                  type="button"
            ...
            <button
              ...
              disabled={loading}
              id="auth-submit-btn"
              className="w-full py-3 text-sm font-semibold text-white bg-gradient-to-r from-indigo-600 to-indigo-600 hover:from-indigo-700 hover:to-indigo-700 rounded-xl transition-all shadow-md shadow-indigo-500/25 hover:shadow-indigo-500/35 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {loading ? (
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-white dark:bg-slate-950 px-2 text-slate-500 uppercase tracking-widest font-bold">Or continue with</span>
            </div>
          </div>
